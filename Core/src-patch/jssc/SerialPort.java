...
/**
 * Patched by Terry Packer to adjust LinuxEventThread polling speed Jan 2, 2019
 * @author scream3r
 */
public class SerialPort {
    ...
    public void addEventListener(SerialPortEventListener listener) throws SerialPortException {
        addEventListener(listener, MASK_RXCHAR, false, 0, 100);
    }
    ...
    public void addEventListener(SerialPortEventListener listener, int mask) throws SerialPortException {
        addEventListener(listener, mask, true, 0, 100);
    }
     ...
     * this method, to do it use variables with prefix <b>"MASK_"</b> for example <b>"MASK_RXCHAR"</b>
     * 
     * The event listener thread will sleep for pollDelayMillis + pollDelayNanos after each read of the events 
     *
     * @see #setEventsMask(int) setEventsMask(int mask)
     *
     * @throws SerialPortException
     */
    public void addEventListener(SerialPortEventListener listener, int mask, int pollDelayMillis, int pollDelayNanos) throws SerialPortException {
        addEventListener(listener, mask, true, pollDelayMillis, pollDelayNanos);
    }
     ...
     * @throws SerialPortException
     */
    private void addEventListener(SerialPortEventListener listener, int mask, boolean overwriteMask, int pollDelayMillis, int pollDelayNanos) throws SerialPortException {
        ...
        if(!eventListenerAdded){
            ...
            eventThread = getNewEventThread(pollDelayMillis, pollDelayNanos);
     ...
     * @since 0.8
     */
    private EventThread getNewEventThread(int pollDelayMillis, int pollDelayNanos) {
        if(SerialNativeInterface.getOsType() == SerialNativeInterface.OS_LINUX ||
           SerialNativeInterface.getOsType() == SerialNativeInterface.OS_SOLARIS ||
           SerialNativeInterface.getOsType() == SerialNativeInterface.OS_MAC_OS_X){//since 0.9.0
            return new LinuxEventThread(pollDelayMillis, pollDelayNanos);
        }
    ...
    private class LinuxEventThread extends EventThread {
        ...
        protected final Log LOG = LogFactory.getLog(LinuxEventThread.class);
        ...
        //Sleep time for event ioctl polling millis + nanos
        private final int pollDelayMillis;
        private final int pollDelayNanos; 

        //Need to get initial states
        public LinuxEventThread(int pollDelayMillis, int pollDelayNanos){
            this.pollDelayMillis = pollDelayMillis;
            this.pollDelayNanos = pollDelayNanos;
        ...
        public void run() {
            while(!super.threadTerminated){
                ...
                try {
                    Thread.sleep(pollDelayMillis, pollDelayNanos);
                }
                catch (Exception ex) {
                    //Do nothing
                }
